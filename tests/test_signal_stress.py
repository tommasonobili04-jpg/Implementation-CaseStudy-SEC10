...
def test_signal_and_stress_scaffold():
    ...
    # PnL vol from toy paths and hedge (lambda=0, zero theta) -> ~0
    ...
    # Sizing
    s_k = size_kelly(edge_ac, sigma_unit + 0.1, kelly_fraction=0.25)  # evita zero denom
    s_s = size_sharpe(edge_ac, sigma_unit + 0.1, inv_sr=0.5)
    assert s_k >= 0.0 and s_s >= 0.0

    # Stress runner con stub pipeline_fn
